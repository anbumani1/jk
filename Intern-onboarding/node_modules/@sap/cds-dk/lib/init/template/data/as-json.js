...
function dataForField(element, artifact, csn, record, data, parent, depth, referenceData) {
  ...
  switch (type) {
    ...
    case 'cds.hana.REAL':
    case 'cds.Decimal': { // Decimal(9,5), Decimal(9), Decimal(precision:5)
      const scale = element.scale ?? (element.precision ? 0 : 2) // scale is 0 if only precision is given
      const prec  = element.precision ? Math.min(Math.pow(10, element.precision-scale), Number.MAX_SAFE_INTEGER) : 100
